...
def content(html_document, url_label):
    label = url_label
    content_match = []
    ...
    for oid_aid in content:
        maked_url = "https://sports.news.naver.com/" + label + "/news/read.nhn?oid=" + oid_aid[0] + "&aid=" + oid_aid[1]
...
def Clearcontent(text):
    ...
    remove_flasherror = re.sub('본문 내용|TV플레이어| 동영상 뉴스|flash 오류를 우회하기 위한 함수 추가fuctio flashremoveCallback|tt|t|앵커 멘트|xa0', '', remove_author)
    ...
    for i in range(0, len(remove_strip)):
        if reverse_content[i:i+2] == '.다':  # reverse 된 기사 내용중, ".다"로 끝나는 경우 기사 내용이 끝난 것이기 때문에 기사 내용이 끝난 후의 광고, 기자 등의 정보는 다 지운다.
    ...
    for year in range(startyear, lastyear + 1):
        ...
        for Month in range(startmonth, lastmonth + 1):
            for Month_Day in range(1, calendar.monthrange(year, Month)[1] + 1):
                ...
                totalpage = javascript_totalpage(url) # TotalPage 확인
...
for url_label in Url_category:  # URL 카테고리
    ...
    for list_page in final_urlday: # Category Year Month Data Page 처리 된 URL
        ...
        for content_page in content(document_content, url_label):
            sleep(0.01)
